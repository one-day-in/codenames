import{b as f,c as L,d as b,R as m}from"./revealedCells-DwXIVMT6.js";import{t as g,d as k,D as w,b as E}from"./url-BkejxNIu.js";import{a as y}from"./roomRepository-DOzx6gVt.js";function G(i,n){var a;(a=i.querySelector(".turn-transition-overlay"))==null||a.addEventListener("click",()=>{n()})}function T(i){const n=g(i);let a=[];async function r(){var e;try{(e=screen.orientation)!=null&&e.lock&&await screen.orientation.lock("portrait")}catch{}}function d(){let e=document.getElementById("orientation-guard");return e||(e=document.createElement("div"),e.id="orientation-guard",e.className="orientation-guard",e.setAttribute("aria-live","polite"),e.innerHTML=`
...
`,"<br>")}async function $(i,{roleType:n,invalidParamsHtml:a}){const{roomId:r,token:d,team:t}=k();if(!r||!d||!t||t!=="resonant"&&t!=="dissonant")return i.innerHTML=a,null;const{room:o,error:l}=await y(r,d),s=(o==null?void 0:o.language)||w,e=g(s);if(!o||l)return i.innerHTML=`<div class="waiting-screen">
...
`,"<br>")}</p>
            ...
            </div>`,await new Promise(p=>{document.getElementById("forceJoinBtn").addEventListener("click",p,{once:!0})})),u.join(c),T(s),document.body.classList.add(`team-${t}`);const v=b(r);return await v.init(),await f({presence:u,store:v,role:c}),{presence:u,store:v,team:t,roomId:r}}export{G as b,$ as i};
