import{C as G,m as ee,e as U,r as V,a as K,g as ne,c as te,p as q,d as re,i as ie,o as oe,f as ae,n as Q,j as F,k as z,q as le,l as se}from"./turnTransitionDismiss-DwWN1bON.js";import{t as R,e as H,I as x,f as ce,h as de,k as ue,D,n as me}from"./roomRepository-DF5mcqIV.js";import"./entry-DRWmpxeO.js";function _e(e){const n=R(e);let s=[];function l(){const i=Math.min(window.innerWidth,window.innerHeight),t=Math.max(window.innerWidth,window.innerHeight);return window.matchMedia("(pointer: coarse)").matches&&i<=768&&t<=1024}async function _(){var i;if(l())try{(i=screen.orientation)!=null&&i.lock&&await screen.orientation.lock("portrait")}catch{}}function d(){let i=document.getElementById("orientation-guard");return i||(i=document.createElement("div"),i.id="orientation-guard",i.className="orientation-guard",i.setAttribute("aria-live","polite"),i.innerHTML=`
            ...
            >
                ...
                <span class="controller-role-picker__state ${m?"controller-role-picker__state--current":""}">${$}</span>
            </button>
        `}return`
        <div class="controller-role-picker" data-controller-role-picker>
            <div class="controller-role-picker__panel ${d?"controller-role-picker__panel--reconnect":""}">
                ...
                <p class="controller-role-picker__summary">${_} / ${G.length} ${l.waitingPlayers}</p>
                <div class="controller-role-picker__grid">
                    ${o.map(t=>{const m=H(t,e),k=G.filter(p=>p.team===t);return`
    ...
    `}function fe({state:e,lang:n,team:s,maxHintButtons:l,teamEffects:_,suppressedTransitionId:d,controllerRolePickerHtml:g=""}){const u=R(n),v=e.turn,o=v.team===s,i=v.guideLimit!==null,t=o&&!i&&!e.gameOver&&!e.turnTransition,m=_.allowTwoWordClue,k=_.forcedGuideLimit,p=_.hideEnemyColors,f=_.hideNightmare,$=H(s,n),y=t?`${u.guide}: ${u.chooseLimit}`:`${u.guide}: ${x.eyeClosed}`,C=[t&&k===1?u.guideForcedOneMove:null,t&&m?u.guideTwoWordClue:null,p?u.guideBlurActive:null,f?u.guideNightmareHidden:null].filter(Boolean),B=C.length?`<div class="guide__meta ${t?"guide__meta--active":"guide__meta--muted"}">${C.join(" • ")}</div>`:"",M=Array.from({length:l},(b,T)=>{const S=T+1;return`
            <button
        ...
        <div class="screen-layout guide-layout">
            <header class="screen-header">
                <div class="guide__header">
                    <div class="guide__title ${t?"guide__title--active":"guide__title--muted"}">${$}</div>
                    <div class="guide__meta ${t?"guide__meta--active":"guide__meta--muted"}">${y}</div>
                    ${B}
            ...
            <main class="screen-body">
                <div class="guide guide--${s}">
                    <div class="grid grid--5">
                        ${e.cells.map((b,T)=>`
                            <div class="${ee(b,{team:s,effects:_})}" data-index="${T}">
    ...
    `}function we({state:e,lang:n,team:s,suppressedTransitionId:l,controllerRolePickerHtml:_=""}){const d=R(n),g=e.turn,u=g.team===s,v=g.guideLimit!==null,o=u&&v&&!e.gameOver&&!e.turnTransition,i=v?Math.max((g.guideLimit??0)-(g.dreamwalkerMoves??0),0):0,t=H(s,n),m=o?`${d.dreamwalker}: <span class="walker__moves-value">${ce(i,n)}</span>`:`${d.dreamwalker}: ${x.eyeClosed}`;return`
...
`,"<br>")}function J(e,n=""){return`<div class="waiting-screen">
        ...
        ${n}
    </div>`}async function Ce(e){const{roomId:n,token:s,team:l,roleType:_}=de();if(!n||!s){e.innerHTML=`<div class="waiting-screen">
            <p>${R(D).scanQr}</p>
        </div>`;return}const{room:d,error:g}=await ue(n,s),u=(d==null?void 0:d.language)||D,v=R(u);if(!d||g){e.innerHTML=`<div class="waiting-screen">
            <p>${v.wrongLink.replace(`
`,"<br>")}</p>
        </div>`;return}_e(u);const o=ae(n);await o.init();const i=te(n);let t=ye({team:l,roleType:_});const m=q(t);t&&await i.isRoleTaken(t)&&(e.innerHTML=`
                <div class="waiting-screen">
                    <div class="taken-screen">
                        <p class="taken-screen__icon">🔒</p>
                        <p class="taken-screen__text">${Te(m,v,u)}</p>
                        <button class="ui-btn" id="forceJoinBtn">${v.forceRejoin}</button>
                    </div>
                </div>`,await new Promise(a=>{var c;(c=document.getElementById("forceJoinBtn"))==null||c.addEventListener("click",a,{once:!0})})),t?i.join(t):i.listen(),await re({presence:i,store:o,role:()=>t});let k=!1,p=new Set,f=null,$=he(n),y=!t;ie(e,()=>{var r;return(r=o.getState())==null?void 0:r.turnTransition},async()=>{var a;const r=(a=o.getState())==null?void 0:a.turnTransition;r!=null&&r.id&&(f=r.id,L()),await o.dismissTurnTransition()});function C(){k=!1,p=new Set,f=null}async function B(r){t=r??null,t?await i.setRole(t):await i.setRole(null)}function M(r){const a=(r==null?void 0:r.gameId)??null;if(!a){$=null,I(n,null),y=!t;return}if(!t){y=!0;return}if(!$){$=a,I(n,a);return}a!==$&&!y&&(y=!0,C(),B(null).then(()=>L()))}function b(){return pe({language:o.getLanguage(),presenceState:i.getPresenceState(),currentPresenceRole:t})}function T(r){e.querySelectorAll("[data-controller-role-picker] button[data-role-type][data-team]").forEach(a=>{a.addEventListener("click",async()=>{const c=a.dataset.roleType,h=a.dataset.team,w=Q(c,h);i.getPresenceState()[w]&&t!==w||(y=!1,$=(r==null?void 0:r.gameId)??null,I(n,$),await B(w),L())})})}function S(r,a,c,h){const w=me(r.teamEffects)[c.team],E=F(r.cells);e.innerHTML=fe({state:r,lang:a,team:c.team,maxHintButtons:$e,teamEffects:w,suppressedTransitionId:f,controllerRolePickerHtml:h}),z({root:e,selector:".guide .grid .cell",currentRevealed:E,prevRevealed:p,hasRenderedBoard:k}),p=E,k=!0,requestAnimationFrame(()=>{e.querySelectorAll(".cell").forEach(P=>le(P))}),e.querySelectorAll(".guide__num-btn:not([disabled])").forEach(P=>{P.addEventListener("click",()=>o.setGuideLimit(parseInt(P.dataset.limit,10)))}),T(r)}function O(r,a,c,h){var j;const w=F(r.cells),E=r.turn,P=E.team===c.team,Y=E.guideLimit!==null,Z=P&&Y&&!r.gameOver&&!r.turnTransition;e.innerHTML=we({state:r,lang:a,team:c.team,suppressedTransitionId:f,controllerRolePickerHtml:h}),z({root:e,selector:".walker .grid .cell",currentRevealed:w,prevRevealed:p,hasRenderedBoard:k}),p=w,k=!0,requestAnimationFrame(()=>se(e)),e.querySelectorAll(".cell--clickable").forEach(W=>{W.addEventListener("click",()=>o.reveal(parseInt(W.dataset.index,10)))}),(j=document.getElementById("refreshBtn"))==null||j.addEventListener("click",()=>{Z&&o.endTurn()}),T(r)}function N(r,a){document.body.classList.remove("team-resonant","team-dissonant"),C(),e.innerHTML=J(a,b()),T(r)}function X(r,a){M(r),(!r.turnTransition||r.turnTransition.id!==f)&&(f=null);const c=q(t),h=y||!c;if(!c){N(r,a);return}document.body.classList.remove("team-resonant","team-dissonant"),document.body.classList.add(`team-${c.team}`);const w=h?b():"";if(c.roleType==="guide"){S(r,a,c,w);return}O(r,a,c,w)}function L({state:r,language:a}={}){const c=r??o.getState(),h=a??o.getLanguage();if(!c){C(),e.innerHTML=J(h,t?"":b()),t||T(null);return}X(c,h)}o.subscribe(L),i.onChange(()=>{(y||!t)&&L()}),oe(()=>L()),L()}export{Ce as initController};
